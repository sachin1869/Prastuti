...
const deleteRequest = async (req, res) => {
  ...
  const team = await Teams.findById(request.team._id);
    ...
    // deleting from user.pending_request
...
const acceptRequest = async (req, res) => {
  try {
    ...
    await Teams.findByIdAndUpdate(team._id, {0
      Members: team.Members,
      Member_Count: team.Member_Count + 1,
    });
    
    // removing request from team's pending request
    await Teams.updateOne(
      { _id: request.team._id },
      { $pull: { Pending_Requests: request._id } },
      {
    ...
    // if team length became 3 then remove all request related to this team from team database
